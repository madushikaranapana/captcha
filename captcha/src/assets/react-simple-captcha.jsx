...
export const loadCaptchaEnginge = (numberOfCharacters, backgroundColor = 'white', fontColor = 'black', charMap = '') => {
    ...
    let cleanedCaptcha = removeCharacters(retVal);

    let captcha = cleanedCaptcha;
    ...
    for (let i = 0; i < parseInt(length); i++) {
        ...
        ctx.fillText(cleanedCaptcha[i], heigt_num, Math.round(Math.random() * (15 - 12) + 12));
    }

    document.getElementById("reload_href").onclick = function () {
        loadCaptchaEnginge(captcha_number, backgroundColor, fontColor, charMap);
    }
    LoadCanvasTemplate_HTML = removeCharacters(LoadCanvasTemplate_HTML);
    LoadCanvasTemplateNoReload_HTML = removeCharacters(LoadCanvasTemplateNoReload_HTML);
